...
data class ChamferingPlanSession(
...
object ChamferingSessionLogic {
    fun emptySession(): ChamferingPlanSession = ChamferingPlanSession()

    fun isTimerRunning(sess: ChamferingPlanSession): Boolean = sess.runningSliceStart != null

    fun isProductionInProgress(sess: ChamferingPlanSession): Boolean =
        sess.wallStart != null && sess.wallEnd == null

    fun isTimerPaused(sess: ChamferingPlanSession): Boolean =
        isProductionInProgress(sess) && sess.runningSliceStart == null

    fun timerPhase(sess: ChamferingPlanSession): TimerPhase {
    ...
    fun readNetProductionMs(sess: ChamferingPlanSession, at: Long = System.currentTimeMillis()): Long {
    ...
    fun readPausedAccumMs(sess: ChamferingPlanSession, at: Long = System.currentTimeMillis()): Long {
    ...
    fun reconcileInProgressTimer(sess: ChamferingPlanSession, now: Long = System.currentTimeMillis()) {
    ...
    fun netProductionSeconds(sess: ChamferingPlanSession, at: Long = System.currentTimeMillis()): Int =
        (readNetProductionMs(sess, at) / 1000).toInt().coerceAtLeast(0)

    fun pausedAccumSeconds(sess: ChamferingPlanSession, at: Long = System.currentTimeMillis()): Int =
    ...
    fun hydrateFromRow(sess: ChamferingPlanSession, row: ChamferingManagementRowDto) {
    ...
    fun operationDisplayMs(sess: ChamferingPlanSession, at: Long = System.currentTimeMillis()): Long {
...
data class PersistedChamferingPlanSession(
...
fun ChamferingPlanSession.toPersisted(): PersistedChamferingPlanSession = PersistedChamferingPlanSession(
...
fun PersistedChamferingPlanSession.applyTo(sess: ChamferingPlanSession) {
