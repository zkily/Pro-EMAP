...
import com.example.smart_emap.data.model.KanbanBatchIssueResponse
import com.example.smart_emap.data.model.KanbanIssueResponse
...
interface CuttingInstructionApiService {
    ...
    suspend fun issueKanban(@Path("id") id: Int): KanbanIssueResponse

    @POST("/api/plan/kanban-issuance/{id}/reissue")
    suspend fun reissueKanban(@Path("id") id: Int): KanbanIssueResponse

    @POST("/api/plan/kanban-issuance/batch-issue")
    suspend fun batchIssueKanban(@Body body: KanbanBatchIssueBody): KanbanBatchIssueResponse
