...
object CuttingProductivityReportLogic {
    ...
    fun buildPrintHtml(
        command: CuttingProductivityReportCommand,
        data: CuttingProductivityAnalysisDataDto,
        ctx: CuttingProductivityPrintContext,
    ): String = when (command) {
        CuttingProductivityReportCommand.PRINT_FULL -> buildFullPrintHtml(ctx)
        CuttingProductivityReportCommand.PRINT_DAILY -> documentShell(
            filters = ctx.filters,
            sectionTitle = "日別推移",
        ...
        CuttingProductivityReportCommand.PRINT_OPERATOR -> {
        ...
        CuttingProductivityReportCommand.PRINT_PRODUCT -> {
        ...
        CuttingProductivityReportCommand.PRINT_PRODUCT_RANK -> documentShell(
            filters = ctx.filters,
    ...
    fun buildDailyBatchPrintHtml(
        ...
        items: List<CuttingDailyBatchPrintItem>,
    ): String {
        ...
        val body = items.mapIndexed { idx, item ->
            ...
            val avgEfficiency = CuttingProductivityLogic.periodAvgEfficiencyFromBucket(sumActualQty, totalSec)
            val badges = buildString {
                append(panelBadge("${daily.size} 日", "soft"))
                if (sumActualQty > 0) append(panelBadge("生産合計 ${CuttingProductivityLogic.fmtInt(sumActualQty)}", "product"))
                if (avgEfficiency != null) {
                    append(panelBadge("平均能率 ${CuttingProductivityLogic.fmtEfficiency(avgEfficiency)} 本/時", "inspector"))
                }
            }
            val panel = panelSection(
                title = "日別推移",
                titleInline = item.lineLabel,
                theme = "chart",
                badges = badges,
                chartFileName = item.chartFileName,
                chartAlt = "日別推移 — ${item.lineLabel}",
                chartTall = true,
                tableHtml = "",
                flowBreak = true,
            )
    ...
    fun buildOperatorProductBatchPrintHtml(
        filters: CuttingProductivityReportFilters,
        items: List<Pair<String, List<CuttingOperatorProductDisplayRow>>>,
    ): String {
        val printedAt = printedAt()
        val body = items.mapIndexed { idx, (label, rows) ->
            ...
            val badges = buildString {
                ...
                append(panelBadge("セッション ${CuttingProductivityLogic.fmtInt(sessionCount)}", "soft"))
                if (sumActualQty > 0) append(panelBadge("生産合計 ${CuttingProductivityLogic.fmtInt(sumActualQty)}", "product"))
                if (avgEfficiency != null) {
                    append(panelBadge("平均能率 ${CuttingProductivityLogic.fmtEfficiency(avgEfficiency)} 本/時", "inspector"))
                }
    ...
    private fun buildFullPrintHtml(ctx: CuttingProductivityPrintContext): String {
    ...
    private fun operatorSectionBadges(ctx: CuttingProductivityPrintContext): String = buildString {
        append(panelBadge("${ctx.operatorRows.size} ライン", "soft"))
        ctx.operatorSectionAvgEfficiency?.let { avg ->
            append(panelBadge("平均能率 ${CuttingProductivityLogic.fmtEfficiency(avg)} 本/時", "inspector"))
        }
    }

    private fun productSectionBadges(ctx: CuttingProductivityPrintContext): String = buildString {
        append(panelBadge("${ctx.productRows.size} 品目", "soft"))
        if (ctx.productSectionTotalQty > 0) {
            append(panelBadge("生産合計 ${CuttingProductivityLogic.fmtInt(ctx.productSectionTotalQty)}", "product"))
        }
    }

    private fun buildProductRankSectionBody(ctx: CuttingProductivityPrintContext): String {
        ...
        val selectedBody = if (selected != null) {
            ...
            buildProductRankHeroHtml(selected, stats) +
                panelSection(
                    title = "ライン別能率",
                    theme = "rank",
                    badges = panelBadge("$operatorCount ライン", "rank"),
    ...
    private fun productRankStats(ranking: CuttingProductivityProductRankingDto): ProductRankPrintStats {
    ...
    private fun buildOperatorPrintTable(
        rows: List<CuttingProductivityOperatorRowDto>,
        maxRows: Int = 0,
    ): String {
        ...
        val body = display.mapIndexed { index, row ->
            val avg = CuttingProductivityLogic.periodAvgEfficiencyFromBucket(row)
            """<tr>
              <td class="center">${operatorRowRankHtml(index)}</td>
              <td>${esc(row.operatorName ?: "—")}</td>
              <td class="num">${esc(CuttingProductivityLogic.fmtInt(row.sessionCount))}</td>
              <td class="num">${esc(CuttingProductivityLogic.fmtInt(row.sumActualQty))}</td>
              <td class="num warn">${esc(CuttingProductivityLogic.fmtPct(row.defectRatePercent))}</td>
              <td class="num"><span class="pill pill--operator">${esc(CuttingProductivityLogic.fmtEfficiency(avg))}</span></td>
            </tr>"""
        }.joinToString("")
        ...
        return """<table class="data data--operator data--compact">
          <thead>${tableHead(listOf("#", "ライン", "件", "生産", "不良率", "平均能率"))}</thead>
          <tbody>$body</tbody>
        </table>$more"""
    }

    private fun buildProductPrintTable(rows: List<CuttingProductivityProductRowDto>): String {
        if (rows.isEmpty()) return emptyNote()
        val body = rows.joinToString("") { row ->
            ...
            """<tr>
              ...
              <td class="num">${esc(CuttingProductivityLogic.fmtInt(row.sumActualQty))}</td>
              <td class="num warn">${esc(CuttingProductivityLogic.fmtPct(row.defectRatePercent))}</td>
              <td class="num"><span class="pill pill--product">${esc(CuttingProductivityLogic.fmtEfficiency(avg))}</span></td>
            </tr>"""
        }
    ...
    private fun buildOperatorProductPrintTable(rows: List<CuttingOperatorProductDisplayRow>): String {
        if (rows.isEmpty()) return emptyNote()
        val body = rows.joinToString("") { row ->
            """<tr>
              <td><span class="product-cd">${esc(row.productCd)}</span></td>
              ...
              <td class="num">${esc(CuttingProductivityLogic.fmtInt(row.sumActualQty))}</td>
              <td class="num warn">${esc(CuttingProductivityLogic.fmtPct(row.defectRatePercent))}</td>
              <td class="num"><span class="pill pill--product">${esc(CuttingProductivityLogic.fmtEfficiency(row.avgEfficiencyPerHour))}</span></td>
            </tr>"""
        }
        return """<table class="data data--product">
          <thead>${tableHead(listOf("CD", "製品名", "件", "生産", "不良率", "能率"))}</thead>
    ...
    private fun buildProductRankOperatorTable(rows: List<CuttingProductivityOperatorRowDto>): String {
        if (rows.isEmpty()) return emptyNote()
        val body = rows.joinToString("") { row ->
            """<tr>
              ...
              <td class="num">${esc(CuttingProductivityLogic.fmtInt(row.sessionCount))}</td>
              <td class="num">${esc(CuttingProductivityLogic.fmtInt(row.sumActualQty))}</td>
              <td class="num"><span class="pill pill--rank">${esc(CuttingProductivityLogic.fmtEfficiency(row.efficiencyPerHour))}</span></td>
              <td class="num warn">${esc(CuttingProductivityLogic.fmtPct(row.defectRatePercent))}</td>
            </tr>"""
        }
        return """<table class="data data--rank">
          <thead>${tableHead(listOf("順位", "ライン", "件", "生産", "能率", "不良率"))}</thead>
          <tbody>$body</tbody>
        </table>"""
    }

    private fun buildProductRankOverviewTable(rows: List<CuttingProductivityProductRankingDto>): String {
        if (rows.isEmpty()) return emptyNote()
        val body = rows.joinToString("") { row ->
            """<tr>
              ...
              <td class="num"><span class="pill pill--rank">${esc(CuttingProductivityLogic.fmtEfficiency(row.topEfficiencyPerHour))}</span></td>
              <td class="num">${esc(row.rankedOperatorCount ?: 0)}</td>
              <td class="num">${esc(CuttingProductivityLogic.fmtInt(row.sumActualQty))}</td>
            </tr>"""
        }
        return """<table class="data data--rank-overview">
          <thead>${tableHead(listOf("CD", "製品名", "TOPライン", "能率", "対象人数", "生産"))}</thead>
          <tbody>$body</tbody>
        </table>"""
    }

    private fun buildProductRankHeroHtml(
        ranking: CuttingProductivityProductRankingDto,
      ...
      <div class="rank-hero__stats">
        <div class="rank-hero__stat"><span class="rank-hero__stat-label">生産合計</span><span class="rank-hero__stat-val">${esc(CuttingProductivityLogic.fmtInt(ranking.sumActualQty))}</span></div>
        <div class="rank-hero__stat"><span class="rank-hero__stat-label">ライン</span><span class="rank-hero__stat-val">${esc(stats.operatorCount)}<small>ライン</small></span></div>
        <div class="rank-hero__stat rank-hero__stat--accent"><span class="rank-hero__stat-label">TOP能率</span><span class="rank-hero__stat-val">${esc(CuttingProductivityLogic.fmtEfficiency(stats.topEfficiency ?: ranking.topEfficiencyPerHour))}<small>本/時</small></span></div>
        <div class="rank-hero__stat"><span class="rank-hero__stat-label">平均能率</span><span class="rank-hero__stat-val">${esc(CuttingProductivityLogic.fmtEfficiency(stats.avgEfficiency))}<small>本/時</small></span></div>
      </div>
    </div>"""
...
<head>
  <meta charset="UTF-8" />
  <title>切断工程 — 生産性分析$titleSuffix</title>
  <style>${printStyles(mode, orientation)}</style>
</head>
<body>
  <header class="hd">
    <div class="hd__title">切断工程 — 生産性分析</div>
    $sectionHtml
    $metaHtml
  </header>
  $kpiHtml
  $body
  <footer class="ft">Smart-EMAPs · 切断生産性分析 · ${esc(printedAt)}</footer>
</body>
</html>"""
    }
...
<body>
  ...
  <footer class="ft">Smart-EMAPs · 切断生産性分析 · ${esc(printedAt)}</footer>
    ...
    <div class="hd__title">切断工程 — 生産性分析</div>
    <div class="hd__section">日別推移（ライン別）</div>
    ...
    <div class="hd__title">切断工程 — 生産性分析</div>
    <div class="hd__section">ライン別製品別</div>
    ${metaLineHtml(filters, printedAt, compact = true)}
  </header>"""
    ...
    private fun metaLineHtml(
      ...
      <span><span class="meta-line__label">出力日時</span> ${esc(printedAt)}</span>
      <span class="meta-line__sep">|</span>
      <span><span class="meta-line__label">ライン</span> ${esc(filters.lineLabel)}</span>
      <span class="meta-line__sep">|</span>
