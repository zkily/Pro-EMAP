...
data class PartOrderPrintFormUi(
...
data class PartManualOrderFormUi(
    val date: String = "",
    val partCd: String = "",
    val partName: String = "",
    val orderQuantity: Int = 0,
    val piecesPerBundle: Int = 1,
...
private val PART_ORDER_SHEET_STYLES = """
    html { height: 100%; }
    body {
    ...
    .order-sheet-main { width: 100%; }
    .issued-info { text-align: left; font-size: 9pt; margin-bottom: 1mm; }
    .title {
      text-align: center;
      font-size: 24pt;
      font-weight: bold;
    ...
    .header { margin-bottom: 1mm; position: relative; }
    .recipient-block { margin-bottom: 4mm; margin-top: 4mm; }
    .recipient-block div {
      margin-bottom: 2mm;
      font-size: 18pt;
      font-weight: bold;
    ...
    .sender-block { text-align: right; margin-bottom: 1mm; margin-top: -12mm; }
    .sender-block div { margin-bottom: 2mm; font-size: 10pt; color: #000000; }
    .approval-box {
    ...
    .approval-box table { width: 100%; border-collapse: collapse; margin: 0; }
    .approval-box td {
    ...
    .delivery-info { margin: 5mm 0; display: flex; justify-content: space-between; }
    .delivery-info div { font-size: 13pt; font-weight: bold; color: #000000; }
    ...
    .notes p { margin: 2mm 0; color: #495057; }
    @page { size: A4 portrait; margin: 1cm 0.5cm 0.5cm 0.5cm; }
    @media print {
      html, body { height: auto; min-height: 0; }
      body { margin: 0; padding-top: 0.3cm; }
      .order-sheet {
        position: relative;
        min-height: 275mm;
        padding-bottom: 36mm;
      ...
      .notes { position: absolute; bottom: 0; left: 0; right: 0; }
    }
""".trimIndent()

fun buildPartOrderPrintHtml(
    rows: List<PartOrderPrintRow>,
    deliveryDate: String,
    form: PartOrderPrintFormUi,
): String {
    val sorted = rows.sortedWith(compareBy(String.CASE_INSENSITIVE_ORDER) { it.partName.orEmpty() })
    val totalOrderQty = sorted.sumOf { it.orderQuantity }
    ...
    val tableRows = sorted.joinToString("\n") { row ->
        val partName = row.partName.orEmpty()
        val length = Regex("(\\d{4})$").find(partName)?.groupValues?.getOrNull(1).orEmpty()
        """
        <tr>
          <td class="text-center">${escapeHtml(row.standardSpec.orEmpty())}</td>
          <td class="text-center">${escapeHtml(partName)}</td>
    ...
    <div class="order-sheet">
      ...
      <div class="summary-row">
        <div class="summary-item">注文本数計  $totalOrderQty</div>
      </div>
      </div>
      <div class="notes">
