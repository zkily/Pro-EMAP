...
object WeldingHistoryRowFormat {
    ...
    /** 確定実績表：welding_management.mes_production_started_at */
    fun formatProductionStart(row: WeldingManagementRowDto): String =
        formatRecordTime(row.mesProductionStartedAt)

    /** 確定実績表：welding_management.mes_production_ended_at */
    fun formatProductionEnd(row: WeldingManagementRowDto): String =
    ...
    fun rowWallElapsedSec(row: WeldingManagementRowDto): Int {
    ...
    /** 確定実績表：一時停止累計（DB 優先） */
    fun rowPausedAccumSec(row: WeldingManagementRowDto): Int {
