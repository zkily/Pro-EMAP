...
fun buildMaterialReceivingHistoryPrintHtml(rows: List<MaterialLogItemDto>): String {
    ...
    val tableRows = rows.joinToString("\n") { row ->
        ...
        <tr>
          ...
          <td class="text-left">${escapeReceivingHtml(row.materialName.orEmpty().ifBlank { "-" })}</td>
    ...
    <table class="print-table">
      <thead>
        <tr>
          <th>日付</th><th>時間</th><th>項目</th><th>材料名</th><th>製造番号</th>
    ...
    return """
    <!DOCTYPE html>
    <html lang="ja"><head><meta charset="UTF-8"/><title>材料受入履歴</title>
    <style>
