...
@Composable
fun EquipmentEfficiencyMasterScreen(viewModel: EquipmentEfficiencyMasterViewModel) {
        ...
        EePageBackground {
            BoxWithConstraints(
                ...
                Column(Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(5.dp)) {
                    EquipmentEfficiencyHeroBar(
                        tabCountsAll = uiState.tabCountsAll,
                        machineCount = uiState.machineDistinctCount,
                        productCount = uiState.productDistinctCount,
                        layout = layout,
                    )
                    EeWorkspace(Modifier.weight(1f)) {
                        Column(Modifier.fillMaxSize()) {
                            EquipmentEfficiencyToolbar(
                            ...
                            EquipmentEfficiencyProcessTabs(
                                activeTab = uiState.activeProcessTab,
                                tabCounts = uiState.tabCounts,
                                onTabSelected = viewModel::setProcessTab,
                            )
                            EquipmentEfficiencyTableSection(
                                rows = uiState.rows,
                                loading = uiState.isLoading,
                                total = uiState.total,
                                ...
                                activeTab = uiState.activeProcessTab,
                                statusUpdatingId = uiState.statusUpdatingId,
                                canEdit = canEdit,
                                canDelete = canDelete,
                                layout = layout,
                                ...
                                onStatusChange = { row, enabled -> viewModel.toggleStatus(row, enabled, canEdit) },
    ...
    uiState.formRow?.let { row ->
        if (uiState.showForm) {
            EquipmentEfficiencyFormDialog(
                ...
                onMachineSelected = viewModel::onMachineSelected,
    ...
    uiState.pendingDeleteId?.let {
        AlertDialog(
            ...
            title = { Text("確認", fontWeight = FontWeight.Bold, fontSize = 15.sp) },
            text = { Text("この能率設定を削除しますか？", fontSize = 13.sp) },
            confirmButton = {
                TextButton(onClick = viewModel::confirmDelete) {
                    Text("削除", color = EeTheme.Rose, fontWeight = FontWeight.SemiBold)
                }
            },
            dismissButton = {
